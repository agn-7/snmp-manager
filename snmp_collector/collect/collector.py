...
class SNMPReader(object):
    ...
    async def read_async_full(
        self, loop, community_data, udp_transport_target, object_type, **kwargs
    ):
        """
        ...
        try:
            ...
            if error_indication:
                str_error = (
                    f"tag_name: {name} - OID: {oid} - IP: {address} \n "
                    f"{error_indication}"
                )
                log.err(str_error)
                data = -8555
            ...
            self.response.publish(module=module, meta_data=meta_data, **result)
            toc = time.time() - tic
